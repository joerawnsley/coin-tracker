...
encoded_user_credentials = base64.b64encode(
    test_user_credentials.encode("utf-8")
).decode("utf-8")
...
def test_list_duties_for_coin_admin(full_database):
    client.put("/api/coins/deeper/add-duties", json=[10, 11, 12], headers=admin_headers)
    response = client.get("/api/coins/deeper/list-duties")
...
def test_add_coin_with_duties_admin(db_with_duties_but_no_coins):
    ...
    new_coin_duties = [
        duty.duty_number for duty in new_coin.duties.order_by(Duty.duty_number)
    ]
...
def test_add_duty_to_coin_user(full_database):
    ...
    client.put("/api/coins/automate/add-duties", json=[1, 2, 3], headers=test_headers)

    automate_coin = Coin.get(Coin.coin_path == "automate")
    automate_duties = {duty.duty_number for duty in automate_coin.duties}
    assert automate_duties == set()


def test_add_duty_to_coin_admin(full_database):

    automate_coin = Coin.get(Coin.coin_path == "automate")
    automate_duties = {duty.duty_number for duty in automate_coin.duties}
    assert automate_coin.duties == set()

    client.put("/api/coins/automate/add-duties", json=[1, 2, 3], headers=admin_headers)
...
def test_remove_duties_from_coin_user(full_database):
    ...
    client.put("/api/coins/houston/remove-duties", json=[5, 7], headers=test_headers)
...
def test_remove_duties_from_coin_admin(full_database):
    houston = Coin.get(Coin.coin_name == "Houston, Prepare to Launch")
    duty_5 = Duty.get(Duty.duty_number == 5)
    duty_7 = Duty.get(Duty.duty_number == 7)
    duty_10 = Duty.get(Duty.duty_number == 10)
    ...
    client.put("/api/coins/houston/remove-duties", json=[5, 7], headers=admin_headers)
...
# DELETE /coins
def test_delete_coin_user_role(full_database):
    coins_in_db = {coin.coin_name for coin in Coin.select()}
    assert coins_in_db == {
        "Automate",
...
def test_delete_coin_admin_role(full_database):
    coins_in_db = {coin.coin_name for coin in Coin.select()}
    assert coins_in_db == {
        "Automate",
        "Houston, Prepare to Launch",
